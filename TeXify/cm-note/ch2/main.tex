...
\mkpt{B 13} We can generate the following list of basic components: 
\begin{tabular}{c|l}
    $a_n$ & $a_n-a_{n-1}$ \\
...
\mkpt{B 14} According to geometric series formula, we have 
$k=\sum_{1\leq j\leq k}^{}1$, gives that 
\[
    \sum_{1\leq j\leq n}^{}(2^{n+1} =2^j)=n_2^{n+1}-(2^{n+1}-2).
\]

\mkpt{B 15} We have $c_n + s_n = \left(\sum_{k=1}^{n}k\right)^2+s_n.$ 
according to the instruction given. 

\mkpt{B 16} Multiplying for both sides will yield the answer. 
That is ${\fp{x}{m}}{\fp{(x-m)}{n}}={\fp{x}{n}}{\fp{(x-n)} m}$. And 
by definition, this two will always equal. 

\mkpt{B 17} Without the loss of the generality, we can prove the 
equation that 
\[
...
Assume $x=k$ is true, then consider $x=k+1$. We can prove that
\begin{align*}
