...
% TODO: THIS Is EXERCISE EXAMPLE
%\begin{Exercise}\label{ex:prop-logic}
%        
...
\begin{Exercise}\label{ex:proplogic}
...
\begin{tasks}
	...
	\task 1 is the factorial of 0, and $u$ is the factorial of $x+1$ if $v$ is the factorial of $x$ and $u$ is $v$ times. 
\end{tasks}
...
\begin{theorem}[常见的推理定律]
	...
	(8) 两难推理: $I_{10}: G\lor H,G\to I, H\to I\Rightarrow I$
...
\begin{idea}
	遇见新概念的时候, 可以应该举一些例子辅助理解, 尽可能弄懂来龙去脉. 上面的内容根本不用记忆! 我们会在下面有更加舒适的方法来进行推演. 
\end{idea}
...
如下是若干个可以完成任何规则的: 其中像分数线一样的上方表示的是前提, 下方表示的是结论. 有时候我们可以把它写成一行就成了$\set{\text{前提}}\vdash \text{结论}$.  

\begin{itemize}
	\item Conjunction: $\dfrac{A,B}{A\an B}$;
	\item Simplification: $\dfrac{A\an B}{A}$ and $\dfrac{A\an B}{B}$;
	\item Addition: $\dfrac{A}{A\an B}$ and $\dfrac{B}{A\an B}$;
	\item Disjunctive Syllogism: $\dfrac{A\an B,\no A}{B}$ and $\dfrac{A\an B,\no B}{A}$;
	\item Modus Pones: $\dfrac{A,A\to B}{B}$;
	\item Conditional Proof: $\dfrac{\text{From } A \text{ derive } B}{A\to B}$;
	\item Double Negation: $\dfrac{\no \no A}{A}$;
	\item Contradiction: $\dfrac{A\an \no A}{\text{False}}$;
	\item Indirect proof: $\dfrac{\text{From } A, \text{derived False}}{A}$
\end{itemize}

我们只用上面的一些内容就可以得到更加复杂的推理规则: 比如我们有前提$A\to B,\no A\to A$, 尝试推出结论$B$. 我们可以用上面的推理规则这样推理: 
$$
\begin{array}{llll}
A \rightarrow B & &  P  (1) \\
...
& A & 4, \text { DN } &(5)\\
& B & 1,5, \text { MP } &(6)\\
& \text { False } & 3,6, \text { Contr }&(7) \\
\neg A & & 4-7, \text { IP } &(8)\\
A & & 2,8, \text { MP } &(9)\\
\text { False } & & 8,9, \text { Contr }&(10)
\end{array}
$$
...
\ti{Modus Tollens: mode that denies}
...
& A & 3, \mathrm{DN} &(4)\\
& B & 1,4, \mathrm{MP} &(5)\\
& \text { False } & 2,5, \text { Contr } &(6)\\
\neg A & & 3-6, \mathrm{IP}&(7)
\end{array}
$$
...
$$
\dfrac{A\an C,A\to C,B\to D}{C\an D}
$$
...
\begin{definition}[``任意''符号消去]
	$\forall$-eliminating. 
	$$
	\forall x. \alpha \vdash \alpha[t/x]
	$$
	where $\alpha$ is free for $x$ in $\alpha$. 
	
	The symbol $\alpha[t/x]$ means substituting all the occurrences $t$ in $\alpha$ to $x$.
\end{definition}


\begin{definition}[``任意''符号的引入]
	$\forall$-introducing.
	\begin{align*}
		& [t] & \text{引入变量}t\\
		&\cdots & \text{经过一系列推理} \\
		& \alpha[t/x] & \text{得到的表达式所有的}x\text{都是}t\\
		& \vdash \forall x. \alpha & \text{就可以推出任意了}
	\end{align*}
	where $\alpha$ is free for $x$ in $\alpha$. 
...
\begin{example}
	$\{P(t),\forall x(P(x)\to \lnot Q(x)\} \vdash \lnot Q(t)$
\end{example}

\begin{sol}
	\begin{align*}
& P(t) & P\\
& \forall x(P(x)\to \lnot Q(x)) & P\\
& P(t)\to \lnot Q(t) & \text{Universal Specification}\\
&\lnot Q(t) & T
\end{align*}
\end{sol}
...
&\forall x . P(x) & (P) \\
&{\left[x_{0}\right] } & \text { (引人变量 }) \\
&P\left(x_{0}\right) \rightarrow Q\left(x_{0}\right) & (\forall \text {-elim },(1),(3)) \\
&P\left(x_{0}\right) & (\forall \text {-elim, },(2),(3)) \\
&Q\left(x_{0}\right) & (\rightarrow \text {-elim },(4),(5)) \\
&\forall x . Q(x) & (\forall \text {-intro },(3)-(6))
\end{align*}
\end{sol}

上面我们介绍了如何把含有任意的内容干掉或者引入. 下面我们来看一下关于存在的一些例子: 

\begin{definition}[存在符号的引入]$\exists$-intro: 
$$
{\alpha [{\color{red}t}/x]\over \exists x.\alpha }\quad (\exists x-\text{intro})
$$
where $t$ is free for $\alpha$. 
	
\end{definition}

这个定理的意思是: 如果$\alpha$对于某个项成立, 那么$\exists x.\alpha$ 成立. 

这里的没有被约束, 我们来举一个例子: $P({c})\vdash \exists x.P(x), c$是任意的常元符号: 这样是可以的, 但是注意下面的例子: 
$\forall y. (y = y) \not \vdash  \exists x. \forall y. (x = y) (y\text{ is not free for x in }\alpha)$

存在符号也可以消去. 第一种方式就是上面的反方向的形式. 就是
\begin{definition}[存在符号消去]
$\exists$-elim:
$$
{\exists x.\alpha\over \alpha [{\color{red}x_0}/x] }\quad (\exists x-\text{elim})
$$
...
\begin{definition}[存在量词的消去(2)]
$\exists$-elim:
$$
\begin{array}{ccccr}
&&&{\left[\alpha\left[x_{0} / x\right]\right]} & \text{引入假设} \\
&&&\vdots & \text{经过一些推理过程}\\
&\exists x . \alpha &\left[x_{0}\right] & \beta &\text{可以推出}\beta, \text{结合前提}\\ 
\hline
&&\beta &&\text{就可推出了}\\
\end{array}(\exists -\text { elim })
$$
	
\end{definition}

这个内容说的是什么呢? 考虑我们在高中做题的时候, 对于存在性问题, 我们一般是假设这个存在, 经过一系列推导之后发现确实存在, 于是就说明了这个东西确实是存在的. 更加形式化的就是: 假设$x_0$使得$\alpha$成立, 如果从$\alpha[x_0/x]$可以推导出$\beta$, 那么$\alpha$从可以推导出$\beta$.
...
&\exists x . P(x) & \text { (前提 }) \\
&{\left[x_{0}\right] \quad\left[P\left(x_{0}\right)\right] } & (\text { 引入变量与假设 }) \\
&P\left(x_{0}\right) \rightarrow Q\left(x_{0}\right) & (\forall \text {-elim, }(1),(3)) \\
&Q\left(x_{0}\right) & (\rightarrow \text {-elim, }(3),(4)) \\
&\exists x . Q(x) & (\exists \text {-intro },(5)) \\
&\exists x . Q(x) & (\exists \text {-elim, }(2),(3)-(6))
\end{align*}
\end{sol}
...
\begin{sol}
	\begin{align*}
&\exists x . P(x) & (\text { 前提 }) \\
&\forall x . \forall y .(P(x) \rightarrow Q(y)) & (\text { 前提 }) \\
&{\left[x_{0}\right] \quad\left[P\left(x_{0}\right)\right] } & (\text { 引入变量与假设 }) \\
&\forall y .\left(P\left(x_{0}\right) \rightarrow Q(y)\right) & (\forall \text {-elim, }(2),(3)) \\
&{\left[y_{0}\right] } & (\text { 引入变量 }) \\
&P\left(x_{0}\right) \rightarrow Q\left(y_{0}\right) & (\forall \text {-elim, (4), (5)) } \\
&Q\left(y_{0}\right) & (\rightarrow \text {-elim, (3),(6)) } \\
&Q\left(y_{0}\right) & (\exists \text {-elim, (1), }(3)-(7)) \\
&\forall y . Q(y) & (\forall \text {-intro, }(5)-(8))
\end{align*}
\end{sol}
...
\ti{生成的排列.} 今天我们希望生成$10!$的排列. 比如从$1234567890$一直生成到$0987654321$为止. 
