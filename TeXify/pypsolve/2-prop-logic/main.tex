...
\begin{theorem}
	...
	(1) 全称量词的消去(Universal Specification);
	\begin{itemize}
		\item $\forall x. A(x)\Rightarrow A(y)$
	\end{itemize}
	
	(2) 全称量词的引入(Universal Elimiation);
	\begin{itemize}
		\item 如果$y$是自由的, $A(y)\Rightarrow \forall x. A(x)$
	\end{itemize}
	
	(3) 特称量词的引入(Universal Generization):
	\begin{itemize}
		\item $\exists x.A(x) \Rightarrow A(c)$
	\end{itemize}
	
	(4)存在量词的引入(Universal Specification): 
	\begin{itemize}
		\item $A(c)\Rightarrow \exists x. A(x)$
	\end{itemize}
