...
\begin{definition}
	设$R$是非空集合$A$上的关系, 如果$A$是自反的, 对称的, 以及传递的, 就称作$R$是$A$上的偏序关系. 通常记为$\preceq$. 整个序关系可以写作$(X,\preceq)$
\end{definition}
...
直观上, 我们会认为定义一个偏序集的上述内容是像图\ref{figs:part-ord}这样的.
...
\begin{definition}[偏序集的最大,最小元]
	设$(A,\preceq)$是偏序集, $B$是$A$的非空子集, 如果$\exists b. (b\in B\an \forall x.(x\in B\to x\preceq b))$, 那么称$b$为$B$的最大元素(greatest element), 简称最大元. 如果$\exists b. (b\in B\an \forall x.(x\in B\to b\preceq x))$, 那么称$b$为$B$的最小元素(smallest element), 简称最小元. 
\end{definition}

\begin{definition}[偏序集的上界,下界]
	设$(A,\preceq)$是偏序集, $B$是$A$的任意的一个子集. 
	
	(1) 如果$\exists a.(a\in A\an \forall x.(x\in B\to x\preceq a))$, 则称$a$为$B$的上界(upper bound); 
	
	(2)  如果$\exists a.(a\in A\an \forall x.(x\in B\to a\preceq x))$, 则称$a$为$B$的下界(upper bound); 
	
\end{definition}

\begin{definition}[偏序集的上, 下确界]
...
$S$是有限的偏序集,若$S$的元素个数最多的反链含$k$个元素, 则$S$可以划分为$k$个元素互不相交的链.
\end{theorem} 
